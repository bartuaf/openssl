...
{
    static char count = 1;
    unsigned char cbuf[16000] = {count++};
...
static int execute_test_ktls(int cis_ktls_tx, int sis_ktls_tx)
{
    ...
    if (!cis_ktls_tx) {
        if (!TEST_false(BIO_get_ktls(clientssl->wbio)))
            goto end;
    } else {
        if (!TEST_true(BIO_get_ktls(clientssl->wbio)))
            goto end;
    }

    if (!sis_ktls_tx) {
        if (!TEST_false(BIO_get_ktls(serverssl->wbio)))
            goto end;
    } else {
        if (!TEST_true(BIO_get_ktls(serverssl->wbio)))
            goto end;
    }
...
static int test_ktls_client_server()
{
    return execute_test_ktls(1, 1);
}

static int test_ktls_no_client_server()
{
    return execute_test_ktls(0, 1);
}

static int test_ktls_client_no_server()
{
    return execute_test_ktls(1, 0);
}

static int test_ktls_no_client_no_server()
{
