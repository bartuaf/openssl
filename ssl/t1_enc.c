...
int tls1_change_cipher_state(SSL *s, int which)
{
...
#ifndef OPENSSL_NO_KTLS
    if (s->compress || SSL_TREAT_AS_TLS13(s))
        goto skip_ktls;
    ...
    /* ktls doesn't works with user provided buffers directly */
    ssl3_release_write_buffer(s);

    /* ktls doesn't support renegotiation */
    if (BIO_get_ktls(s->wbio))
        goto err;
    ...
    EVP_CIPHER_CTX_ctrl(dd, EVP_CTRL_GCM_GET_IV,
    ...
    BIO_set_ktls(wbio, &crypto_info, which & SSL3_CC_WRITE);
